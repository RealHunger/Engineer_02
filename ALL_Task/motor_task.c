...
void motor_task_func(void const * argument) {
    ...
    // // 从臂参数
    // joint[0]->init(joint[0], 0x01, ARM_CAN_HANDLE_12,5,0.0f,0.0f,12.5f,45.0f,54.0f);
    // joint[1]->init(joint[1], 0x02, ARM_CAN_HANDLE_12,5,0.0f,0.0f,12.5f,25.0f,200.0f);
    // joint[2]->init(joint[2], 0x03, ARM_CAN_HANDLE_345678,5,0.0f,0.0f,12.5f,10.0f,28.0f);
    // joint[3]->init(joint[3], 0x04, ARM_CAN_HANDLE_345678,5,0.0f,0.0f,12.5f,10.0f,28.0f);
    // joint[4]->init(joint[4], 0x05, ARM_CAN_HANDLE_345678,5,0.0f,0.0f,12.5f,3.0f,10.0f);
    // joint[5]->init(joint[5], 0x06, ARM_CAN_HANDLE_345678,5,0.0f,0.0f,12.5f,3.0f,10.0f);
    // joint[6]->init(joint[6], 0x07, ARM_CAN_HANDLE_345678,5,5.0f,0.0f,12.5f,3.0f,10.0f);
    ...
    while (robot_ctrl.monitor.arm_ready == 0) {
        osDelay(10);
    }
    osDelay(1000);
    ...
    osDelay(50);
    gripper->send_enable_cmd(gripper);
    osDelay(50);
