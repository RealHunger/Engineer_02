...
/******************************************************************************************
 *                                   自定义控制器 宏定义（适配协议0x0302）
 ******************************************************************************************/
#define CUSTOM_CTRL_FRAME_TOTAL_LEN  39u    // 总帧长：5(帧头)+2(命令码)+30(数据)+2(CRC16)=39
#define CUSTOM_CTRL_DATA_LEN         30u    // 数据段长度（协议表1-40要求0x0302对应30字节）
#define CUSTOM_CTRL_CMD_ID           0x0302 // 自定义控制器→机器人命令码（图传链路）
#define CUSTOM_CTRL_JOINT_COUNT     7u     // 关节数量（对应发送端7个关节）
#define CUSTOM_CTRL_SOF              0xA5u  // 帧头起始标识（协议表1-3强制要求）
...
typedef struct {
    struct {
        float joint_angles_rad[CUSTOM_CTRL_JOINT_COUNT]; // 7个关节弧度值
        uint8_t gripper_open;                            // 夹爪状态：0=闭合，1=打开
    } custom_ctrl;
    uint32_t last_update_tick; // 最后更新时间戳
} RC_custom_t;

/******************************************************************************************
 *                                   【核心】合并后的总遥控器结构体（扩展自定义控制器字段）
 ******************************************************************************************/
typedef struct {
    RC_dt7_t    dt7;        // DT7遥控器全部数据
    RC_vt13_t   vt13;       // VT13遥控器全部数据
    RC_custom_t custom;     // 新增：自定义控制器数据（通过VT13图传链路接收）
} RC_ctrl_t;
...
void RC_Init_Custom(void); // 自定义控制器接收初始化
void RC_Init(void);        // 一键初始化双遥控器+自定义控制器
